module.exports = function(RED)
{
	...
	function HueMotion(config)
	{
		...
		this.unsubscribe = bridge.subscribe("motion", config.sensorid, function(info)
		{
		...
		this.on('input', function(msg, send, done)
		{
			...
			if(!tempSensorID)
			{
				scope.error(RED._("hue-motion.node.error-no-id"), msg);
				return false;
			}
			...
			if(!currentState)
			{
				scope.error(RED._("hue-motion.node.error-not-available"), msg);
				return false;
			}
			...
			// TUNE THE SENSITIVITY (CAMERAS AND MOTIONAWARE AREAS)
			if(typeof msg.payload != 'undefined' && typeof msg.payload.sensitivity != 'undefined')
			{
				const maximum = currentState.payload.sensitivityMax;

				if(typeof maximum == 'undefined')
				{
					scope.error(RED._("hue-motion.node.error-no-sensitivity"), msg);
					return false;
				}

				const sensitivity = parseInt(msg.payload.sensitivity);
				if(isNaN(sensitivity) || sensitivity < 0 || sensitivity > maximum)
				{
					scope.error(RED._("hue-motion.node.error-invalid-sensitivity", { max: maximum }), msg);
					return false;
				}

				patchObject["sensitivity"] = { sensitivity: sensitivity };
			}
			...
			if(Object.values(patchObject).length > 0)
			{
				...
				{
					bridge.patch(currentState.info.type, tempSensorID, patchObject)
