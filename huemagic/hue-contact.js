module.exports = function(RED)
{
	...
	function HueContact(config)
	{
		...
		if(bridge == null)
		{
			this.status({fill: "red", shape: "ring", text: "hue-contact.node.not-configured"});
			return false;
		}
		...
		if(!config.sensorid)
		{
			this.status({fill: "grey", shape: "dot", text: "hue-contact.node.universal"});
		}
		...
		if(config.sensorid)
		{
			this.status({fill: "grey", shape: "dot", text: "hue-contact.node.no-contact"});
		}
		...
		this.on('input', function(msg, send, done)
		{
			...
			if(Object.values(patchObject).length > 0)
			{
				...
				if(config.sensorid)
				{
					scope.status({fill: "grey", shape: "ring", text: "hue-contact.node.command"});
				}
				...
				{
					bridge.patch("contact", tempSensorID, patchObject)
